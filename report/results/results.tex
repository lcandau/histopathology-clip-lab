...
\begin{itemize}
    \item Macro-F1 on the LC25000 test split: $0.996$ (saturated). In-distribution does not discriminate the trained variants in this study --- the meaningful comparison is the external evaluation in \S\ref{sec:results-baseline-ood}.
...
\begin{itemize}
    ...
    \item \textbf{Method}: Macenko normalization, fit on a single reference patch and applied deterministically to every training and evaluation image. The reference is the first NORM tile from NCT-CRC-HE-7K (``NCT-NORM''), chosen so LC25000 lands in approximately the same stain space NCT arrives in --- which lets us feed NCT raw at evaluation without a second Macenko pass.
    \item At external evaluation time, Chaoyang and LungHist700 patches (native stain at release) are passed through the same Macenko(NCT-NORM) transform so they land in the model's training-stain space. NCT-CRC patches are fed raw (already approximately in that space at release).
    \item Inputs are cached on Drive after one-time normalization so re-runs of the downstream model are cheap.
    \item Reinhard and Vahadane were also explored as alternative normalizations in an exploratory pass but did not progress to the full external-evaluation ablation: Vahadane's NMF solver fails to converge on a fraction of LC25000 patches (visible in Figure~\ref{fig:stain-before-after}(d)) and Reinhard is a Lab-space colour-statistics transfer rather than a stain-decomposition method, included in the figure for visual comparison only.
\end{itemize}
...
\subsection{Training source results}
\begin{itemize}
    \item Macro-F1 on LC25000 with Macenko(NCT-NORM) at training: $0.986$, saturated alongside baseline. In-distribution does not discriminate this lever (or any other trained variant in this study).
\end{itemize}

\subsection{Generalization to external datasets}
\begin{itemize}
    \item Three-dataset external evaluation with the Macenko(NCT-NORM) checkpoint, restricted argmax per dataset:
\end{itemize}
...
\begin{table}[ht]
    ...
    LungHist700 (different-organ lung, 20$\times$)  & $0.305$ & $0.325$ & $+0.020$ \\ \hline
    \end{tabular}
    \caption{Macenko(NCT-NORM) vs baseline on the three external evaluation datasets. Macenko helps where the OOD patches arrive in their native stain palette (Chaoyang, LungHist700) and regresses on NCT, which arrives already Macenko-normalised at release.}
    \label{tab:stain-3way}
\end{table}

\begin{itemize}
    \item \textbf{The lever's direction is data-dependent}: it \emph{helps} on the two datasets that arrive in their native stain space (Chaoyang $+0.030$ F1, LungHist700 $+0.020$ F1, both receiving Macenko at inference to land in the training-stain space); it \emph{regresses} on NCT-CRC ($-0.022$ F1), the only dataset that was already partially Macenko-normalised at release.
    \item Mechanism: when the OOD input genuinely sits in a different stain palette than training, Macenko at inference moves it closer to the model's training distribution and the frozen backbone produces better features. When the input is already in (approximately) the target stain space, applying Macenko at training added a non-linear input-statistic shift that the frozen backbone could not absorb, and the eval-time pass-through cannot recover the small cost paid.
    \item \textbf{Magnitude is modest}. The $+0.020$--$+0.030$ F1 gains on Chaoyang and LungHist700 are real but small. By comparison, swapping the image encoder to PLIP (Section~\ref{sec:results-image-encoder}) gains $+0.027$ on Chaoyang (comparable to Macenko) but $+0.335$ on LungHist700 (an order of magnitude larger). Stain normalization is not the dominant lever even where it helps.
\end{itemize}

\subsection{What we learned}
\begin{itemize}
    \item Stain normalization's effect under shift is \emph{data-dependent}. It transfers where it normalizes away genuine native-stain differences, and regresses where the input was already in the target stain space (here, NCT-CRC's release-time Macenko-normalised state).
    \item Once you commit to Macenko at training time, the right protocol is to apply the same Macenko at evaluation time \emph{only} on OOD inputs that do not arrive in the target stain space already. Doubling normalization on a pre-normalised target is what historically broke the OOD F1 entirely (the ``double-Macenko'' regime we surfaced earlier in the project).
    \item Methodologically: stain normalization is a real, low-cost lever for cross-laboratory histopathology pipelines, but at the frozen-backbone scale evaluated here its gain is dominated by image-encoder pretraining choice on the same datasets.
\end{itemize}
...
\begin{itemize}
    ...
    \item Effect-size summary: PLIP gains over baseline by $+0.067$ F1 on close-shift colon, $+0.027$ on different-hospital colon, \textbf{$+0.335$ on different-organ lung}. DINOv2 gains over baseline by $-0.011$ / $-0.010$ / $+0.152$ on the same three datasets.
