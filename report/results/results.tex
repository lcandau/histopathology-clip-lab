...
\begin{itemize}
    ...
    \item \textbf{On lung (LungHist700)}: both architecture/self-supervision and histopathology pretraining contribute, roughly equally. DINOv2 partially rescues the lung collapse ($+0.152$ F1, from a degenerate predictor to a balanced 3-class classifier with per-class recalls $0.46/0.52/0.40$); PLIP rescues it further ($+0.183$ F1 over DINOv2, to $0.51/0.49/0.91$). \emph{Together these decompose PLIP's lung advantage into two roughly equal halves}: ``architecture + self-supervised pretraining on diverse natural images'' and ``histopathology-specific pretraining''.
