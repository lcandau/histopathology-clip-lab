...
\subsection{Stain normalisation in histopathology images}
...
\end{equation}
where the $+1$ avoids the singularity on pure-black pixels. In OD space, the contributions of the two stains add linearly: each pixel's OD vector is a weighted sum of the per-stain colour vectors, written compactly as the matrix--vector product
\begin{equation}
\mathrm{OD} = S \cdot C,
\end{equation}
with $S \in \mathbb{R}^{3 \times 2}$ the stain-vector matrix (one 3-D colour vector per stain, stacked column-wise) and $C \in \mathbb{R}^{2}$ the per-stain concentration vector. Macenko estimates $S$ from the image by taking the SVD of the OD-space pixel cloud and choosing the two extreme directions in the top-2 singular-vector plane; the concentrations $C$ then follow by pseudo-inversion, $C = S^{+}\, \mathrm{OD}$. To normalise to a reference image with stain matrix $S_\mathrm{ref}$, the concentrations are preserved and re-projected through the reference stains:
\begin{equation}
...
\begin{figure}[H]
    ...
    \end{minipage}
    \caption{Stain normalisation comparison on a single LC25000 benign-lung patch. (a) Raw input, (b) Macenko-normalised, (c) Reinhard-normalised, (d) Vahadane-normalised. Each method produces a different colour palette from the same underlying tissue content. The full multi-class comparison, including the Vahadane convergence failure on some patches, is in §\ref{sec:results-stain}.}
...
A practical consequence is that the CLIP paradigm, a contrastive image--text training framework, is the \emph{object} of study in this work, rather than the classifier per se. Every variant is evaluated via classification F1, but the broader question is what kind of representations the paradigm produces under histopathology supervision, and what changes when individual components of the pipeline are swapped.
...
We use PLIP in two distinct ways. As a \emph{zero-shot reference baseline} (§\ref{sec:results-plip-zeroshot}), the full PLIP model --- image and text encoders both frozen --- is used off-the-shelf for cosine-similarity classification on all four datasets (LC25000 plus the three external evaluation sets). As a \emph{pretrained image encoder} in the image-encoder comparison (§\ref{sec:meth-image-vit}), only PLIP's ViT-B/32 vision encoder is used, paired with our LC25000-trained projection head and a separate BERT text encoder. The two roles isolate different questions: the first tests what pathology pretraining gives on its own; the second tests what pathology pretraining gives on top of the additional LC25000 supervision.


\section{Image Encoders}
\label{sec:meth-image}

The image encoder is the component swapped in the central image-encoder comparison of this work (§\ref{sec:results-image-encoder}), and the choice of which three encoders to compare is what makes that comparison interpretable. Three encoders --- ResNet50 (§\ref{sec:meth-image-resnet}), PLIP's ViT-B/32 (§\ref{sec:meth-image-vit}), and DINOv2 ViT-B/14 (§\ref{sec:meth-image-dinov2}) --- were chosen so that swapping one for another isolates a specific design choice rather than confounding several at once. \textbf{Before describing each encoder in detail, we lay out the three axes those design choices span.}

Each image encoder in this work brings a combination of three independent design choices to the table. The first axis is the \textbf{architecture itself}: a convolutional network (CNN, the classic vision choice) versus a Vision Transformer (ViT, the more recent transformer-based alternative). Different architectures induce different inductive biases (CNNs hard-code spatial locality; ViTs learn it from data) and produce different feature granularities at the same input resolution. The second axis is the \textbf{pretraining objective}: the supervised classification used for ResNet50's ImageNet pretraining, the image--text contrastive used for PLIP, or the self-supervised distillation used for DINOv2. Each objective optimises for different geometric properties of the resulting feature space. The third axis is the \textbf{pretraining domain}: general natural images (ResNet50's ImageNet and DINOv2's LVD-142M) versus histopathology image--text pairs (PLIP's OpenPath). Domain matters because the features a network learns to detect depend on what it was asked to discriminate.

A direct swap from ResNet50 to PLIP changes all three axes simultaneously, so the resulting delta is not attributable to any single one. DINOv2 partially fixes this: inserting it between ResNet50 and PLIP splits the swap into two steps, each bundling a pair of axes rather than all three. ResNet50 $\rightarrow$ DINOv2 changes architecture (CNN $\rightarrow$ ViT) and objective (supervised $\rightarrow$ self-supervised) together while keeping the pretraining domain in general natural images; its delta measures ``switching to a self-supervised ViT'' as a bundle. DINOv2 $\rightarrow$ PLIP keeps the ViT architecture but changes both objective (self-supervised distillation $\rightarrow$ image--text contrastive) and domain (general $\rightarrow$ histopathology); its delta measures ``switching to histopathology pretraining'', with the objective shift folded in. Fully separating objective from domain in a single step would require a fourth encoder we do not include --- for example, a general-domain CLIP-style model trained on natural image--text pairs --- and is left to future work. DINOv2 is, in this sense, the experimental control of the comparison: a partial but useful decomposition rather than a fully factorised one.

A note on the subsections below: ResNet50 and DINOv2 describe full encoder-plus-pretraining units, but the ViT subsection covers only the architecture. The encoders we actually plug into the pipeline are PLIP's ViT-B/32 (already described in §\ref{sec:meth-plip-model}) and DINOv2's ViT-B/14. The two ViTs also differ in patch size (/32 vs /14), giving them different feature granularity at the same input resolution; this becomes a methodological caveat in the §\ref{sec:results-image-encoder} discussion.
...
The text encoder is the second component swapped, in the text-encoder comparison of §\ref{sec:results-text-encoder}. The three encoders compared here are all from the BERT family~\cite{devlin2019bert} (Devlin et al., 2019): the original \textbf{BERT}, \textbf{BioBERT}, and \textbf{PubMedBERT}. In plain terms, BERT is a pre-trained language model: given a short piece of text, it produces a fixed-size numerical vector --- an ``embedding'' --- that downstream layers can read. It learned to do this by reading a large amount of general-English text (BookCorpus and English Wikipedia) during pretraining, with a fraction of the words randomly hidden and the model asked to fill them in from context. That training setup forces BERT to encode each word's meaning in terms of the surrounding sentence rather than in isolation. The baseline is \texttt{bert-base-uncased}, which returns a 768-dimensional vector for each input prompt. The architecture is identical across all three encoders; what differs is the corpus each was pretrained on.

The other two encoders are biomedical variants of the same architecture:
...
Each image encoder expects pixel values in the distribution it was pretrained on, so the preprocessing we apply depends on which backbone we are using (Table~\ref{tab:image-preprocessing}). All inputs are resized to $224 \times 224$ with bilinear interpolation before they hit the encoder. We do not add any further augmentation on top: LC25000 already ships with rotation/flip augmentation at release, and the external evaluation sets are used as-is.
...
The three text encoders compared in §\ref{sec:results-text-encoder} (BERT, BioBERT, PubMedBERT) all use BERT-family WordPiece tokenisers. PLIP's own text encoder, used only inside the frozen zero-shot reference of §\ref{sec:results-plip-zeroshot}, tokenises with CLIP's BPE scheme instead and is not part of this comparison. We pad or truncate every input to a fixed sequence length of 24 tokens, which leaves comfortable headroom for every class-prompt format we test. The baseline's composed prompt template is \texttt{"A histopathology image of \{class\_name\}."}, where \texttt{\{class\_name\}} is replaced at runtime by the specific LC25000 tissue label --- for example, \texttt{"A histopathology image of lung adenocarcinoma."} for that class.
...
The global metrics reported throughout this work are listed below. In all formulas, $N$ is the total number of samples in the evaluation set, $K$ the number of classes (five for LC25000 in-distribution; two or three on the restricted-argmax external evaluations of §\ref{sec:meth-eval-restricted}), $n_k$ the number of samples in class $k$, and $\hat{y}_i, y_i$ the predicted and true label for sample $i$. Per-class true-positive, false-positive, and false-negative counts are $\mathrm{TP}_k, \mathrm{FP}_k, \mathrm{FN}_k$; per-class precision is $P_k = \mathrm{TP}_k / (\mathrm{TP}_k + \mathrm{FP}_k)$. Definitions and implementations follow scikit-learn~\cite{pedregosa2011sklearn} (Pedregosa et al., 2011).

\begin{itemize}
    \item \textbf{Accuracy} --- the fraction of samples whose predicted class matches the true class:
    $$\text{Accuracy} = \frac{1}{N} \sum_{i=1}^{N} \mathds{1}[\hat{y}_i = y_i],$$
    where $\mathds{1}[\cdot]$ is the indicator function (1 if the condition inside holds, 0 otherwise).
    \item \textbf{Balanced accuracy} --- the macro-average of per-class recall: every class is weighted equally regardless of how many samples it has, so a model that always predicts the majority class on imbalanced data cannot do well on this metric:
