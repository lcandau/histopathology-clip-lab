...
%:Override libroETSI.sty's textformat=period — captions in this document end with their own
%:author-supplied period, so we don't want the caption package appending a second one.
...
% Index and Glossary are unused in this TFM — disabled so the TOC does not
% list empty backmatter sections. To re-enable, uncomment \makeindex /
% \makeglossaries and the corresponding \printindex / \printglossaries blocks
% at the end of the document, then add \newacronym/\index entries as needed.
%\makeindex
%\makeglossaries
...
%:Prevent widows (single last line of a paragraph at page top)
%:and orphans (single first line at page bottom) -- needed to stop
%:two-word widows like "LC25000 supervision." landing alone on a page.
\widowpenalty=10000
\clubpenalty=10000
...
\titulo{A Study on Foundational Vision-Language Models \mbox{for}~Histopathology Image Analysis} %\mbox + ~ keep "for Histopathology" on the same line without inserting a stray double-space
\autor{Leandro Candau Sánchez de Ybargüen}
\director{Miguel Ángel Gutiérrez Naranjo}
\titulodirector{Profesor Titular de Universidad}
\directorb{Miguel Cárdenas Montes}
\titulodirectorb{Investigador Científico de OPI}
...
% Suppress the Tribunal sign-off page (tutor feedback 2026-06-22, Miguel Ángel: "Quita esta página").
% The page was meant for the physical-printout era and adds nothing to the digital PDF.
% Overrides \secondpagePFC from edicionPFC.sty with a no-op; template file untouched.
\def\secondpagePFC{}
...
% Index and glossary intentionally omitted — see the \makeindex / \makeglossaries
% block at the top of this file for how to re-enable them.
